...
#endif
#include <WiFiUdp.h>
#include <ArtnetWifi.h> // для управления светодиодной лентой
#define FASTLED_INTERNAL
#include <FastLED.h> // для работы с лентой

//вариант, если работаешь с одной светодиодной палкой
const bool apMode = false; // если apMode = true,то наш контроллер - сервер, он раздает вайфай; если apMode = false, то контроллер подключается к внешней точке доступа
const String apName = "dila"; //имя точки доступа (работает, если apMode = true)
const char WiFiAPPSK[] = "hblg4369"; //пароль (работает, если apMode = true)

// Wifi settings - настройка внешней точки доступа, если apMode = false
const char* ssid = "dila"; //имя внешней точки доступа
const char* password = "hblg4369"; //пароль внешней точки дотупа
...
void setup(){
  Serial.begin(115200);
  Serial.println();
  Serial.print("Setup whith ");Serial.print(maxUniverses);
  if (maxUniverses > 1) Serial.print(" universes"); else Serial.print(" universe");
  ConnectWifi();
